...
class CombinedRadarTester:
    ...
    def get_metrics(self, Y_hat, Y_true):
        ...
        Y_true_extended = F.conv2d(Y_true.unsqueeze(
            1).float(), self.kernel, padding=1).squeeze(1)
        Y_true_extended = (Y_true_extended > 0)
        Y_hat_extended = F.conv2d(Y_hat.unsqueeze(
            1), self.kernel, padding=1).squeeze(1)
        ...
        if frames_with_targets.any():
            ...
            pd = (detected[frames_with_targets] /
                  n_targets[frames_with_targets]).mean()
        else:
    ...
    def find_threshold(self, loader, target_pfa):
        ...
        print(f"Found threshold = {th:.4f}, PFA = {
              pfa_res:.6f} after {cnt} iterations")
...
class CFARTester:
    ...
    def get_metrics(self, detection_surface, Y_true, threshold):
    ...
    def find_threshold(self, loader, target_pfa):
        ...
        while rel_err >= 0.01 and cnt < 30:
            _, pfa_res = self.get_metrics(detection_surface, Y_true, th)
            rel_err = abs(pfa_res - target_pfa) / abs(target_pfa)
    ...
    def evaluate_pd_pfa(self, nu, scnr=0):
        ...
        dataset = ConcatDataset(
            [RadarDataset(4096, 4, False, nu, scnr), RadarDataset(2048, 0, False, nu, scnr)])
        ...
        for target_pfa in tqdm(target_pfas):
            ...
            print(f"Found threshold = {th:.4f}, PFA = {
                  pfa_res:.6f}, after {cnt} iterations")
            pd, pfa = self.get_metrics(detection_surface, Y_true, th)
            results.append((pd, pfa))
    ...
    def evaluate_pd_scnr(self, nu, target_pfa=5e-4):
        ...
        ref_dataset = ConcatDataset(
            [RadarDataset(4096, 4, False, nu, 0), RadarDataset(2048, 0, False, nu)])
        ref_loader = DataLoader(ref_dataset, batch_size=256, num_workers=2, persistent_workers=True,
                                pin_memory=torch.cuda.is_available(), collate_fn=cfar_collate_fn)
        threshold, pfa_res, cnt = self.find_threshold(ref_loader, target_pfa)
        print(f"Found threshold = {threshold:.4f}, PFA = {
              pfa_res:.6f}, after {cnt} iterations")
        scnr_range = np.arange(-40, 11, 5)
        results = []
        ...
        for scnr in tqdm(scnr_range):
            test_dataset = ConcatDataset(
                [RadarDataset(4096, 4, False, nu, scnr), RadarDataset(2048, 0, False, nu)])
            test_loader = DataLoader(test_dataset, batch_size=256, num_workers=2, persistent_workers=True,
                                     pin_memory=torch.cuda.is_available(), collate_fn=cfar_collate_fn)

            detection_surface, Y_true = self.feed_forward(test_loader)
            pd, pfa = self.get_metrics(detection_surface, Y_true, threshold)
            results.append((pd, pfa))
...
def load_trained_models(model_path=("models/range_model.pt", "models/doppler_model.pt")):
    ...
    try:
        range_model.load_state_dict(
            torch.load(model_path[0], weights_only=True))
    ...
    try:
        doppler_model.load_state_dict(
            torch.load(model_path[1], weights_only=True))
...
def test():
    ...
    range_model, doppler_model = load_trained_models(
        ("models/range_model.pt", "models/doppler_model.pt"))
    ...
    plot_pd_pfa(pd_pfa_results, "dafc_pd_pfa")
    ...
    plot_pd_scnr(pd_scnr_results, "dafc_pd_scnr")
...
def test_cfar(detector="CA"):
    ...
    for nu in tqdm(nu_values):
        ...
        pd_pfa_results[nu] = (pd, pfa)
    plot_pd_pfa(pd_pfa_results, detector + "_CFAR_pd_pfa_no_clutter")
    ...
    for nu in tqdm(nu_values):
        # Test CFAR
        pd, pfa, scnr = cfar_tester.evaluate_pd_scnr(nu, target_pfa=5e-4)
        pd_scnr_results[nu] = (pd, pfa, scnr)
    plot_pd_scnr(pd_scnr_results, detector + "_CFAR_pd_scnr_no_clutter")

    save_results(detector + "_CFAR_no_clutter",
                 pd_pfa_results, pd_scnr_results)
