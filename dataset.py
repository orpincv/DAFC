...
class RadarDataset(Dataset):
    def __init__(self, num_samples, n_targets: int = 8, random_n_targets=True, nu=None, scnr=None):
        ...
        self.cn_norm = torch.sqrt(
            self.N * self.K * torch.tensor((self.N // 2) + self.sigma2))
        ...
        # Doppler interval (m/s)
        self.v_min, self.v_max = -(31 * self.vel_res), (31 * self.vel_res)
        # Clutter min/max velocity (m/s)
        self.vc_min, self.vc_max = self.v_min, self.v_max

        # Calculate range and Doppler bins
        self.R = torch.arange(self.r_min, self.r_max +
                              self.rng_res, self.rng_res)
    ...
    def generate_target_signal(self, ranges, velocities, phases, SCNR_dBs):
        ...
        range_steering = torch.exp(-1j *
                                   torch.outer(w_r, torch.arange(self.N)))

        # Doppler steering vector
        w_d = (2 * torch.pi * self.T0 * 2 * self.fc * velocities) / self.c
        doppler_steering = torch.exp(-1j *
                                     torch.outer(w_d, torch.arange(self.K)))

        # Fast-time x Slow-time matrix
        rd_signal = range_steering.unsqueeze(-1) * \
            doppler_steering.unsqueeze(1)

        # Physical Phase
        tau0 = 2 * ranges.unsqueeze(-1).unsqueeze(-1) / self.c
        physical_phase = ((-1j * 2 * torch.pi * self.fc * tau0) +
                          (torch.pi * self.B * tau0 ** 2)) / (self.N * self.fs)
        ...
        rd_signal = (sig_amp.unsqueeze(-1).unsqueeze(-1)
                     * rd_signal).sum(dim=0)
        return rd_signal
    ...
    def generate_clutter(self, nu):
        ...
        # Clutter Doppler shift (m/s)
        fd = (2 * torch.pi * (2 * self.fc * clutter_vel) / self.c)
        sigma_f = 0.05  # From paper
        p, q = torch.meshgrid(torch.arange(
            self.N), torch.arange(self.K), indexing='ij')

        # Generate complex correlation matrix
        M = torch.exp(-2 * torch.pi ** 2 * sigma_f ** 2 *
                      (p - q) ** 2 - 1j * (p - q) * fd * self.T0)

        # Generate complex normal samples
        z = torch.randn(self.K, self.dR, dtype=torch.cfloat) / \
    ...
    def gen_frame_and_labels(self):
        ...
        W = (torch.randn(self.N, self.K, dtype=torch.cfloat) /
             torch.sqrt(torch.tensor(self.sigma2)))
        ...
        if self.with_targets:
            n = torch.randint(1, self.n_targets + 1, (1,)
                              ) if self.random_n_targets else self.n_targets
            ...
            SCNR_dBs = torch.empty(
                n).uniform_(-5, 10) if self.scnr is None else self.scnr * torch.ones(n)
            S = self.generate_target_signal(
                ranges, velocities, phases, SCNR_dBs)
